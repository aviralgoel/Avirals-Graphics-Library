			...
			if (((edge1To2 < 0 && edge2To3 < 0 && edge3to1 < 0) || edge1To2 == 0 || edge2To3 == 0 || edge3to1 == 0)
				...
				if (currentZ >= 0) {
					...
					if (interp_mode == GZ_FLAT) {	// No Shading
						...
						redIntensity = ctoi(GInterPolate(i, j, redA, redB, redC, redD));
						greenIntensity = ctoi(GInterPolate(i, j, greenA, greenB, greenC, greenD));
						blueIntensity = ctoi(GInterPolate(i, j, blueA, blueB, blueC, blueD));
...
void GzRender::SortByXSpecialCases(GzCoord  vertices[3], GzCoord normals[3])
{
	// V1y = V2y ( FLAT TOP )
...
void GzRender::SortByY(GzCoord  vertices[3], GzCoord normals[3])
{
